...
- vault 的 RawMD 注入 Obsidian 块引用 `^sec-NNNN`，Graph/Wiki 证据链接改为 `#^sec-NNNN`，
  点击后可精准跳转到原文具体段落（此前仅跳到文档顶部）。
...
- `docx_v2` adapter 现在会把每张内嵌图片注册为 `AssetRef`。此前适配器
  会写出 `![](assets/img_N.png)` 链接并落盘图片，却返回 `assets=()`，
  导致任何含图片的 DOCX（几乎所有真实工程文档）在确定性核心的 H5
  检查（"markdown 引用的资源未登记"）处失败、无法抽取。
